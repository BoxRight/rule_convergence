...
class BatchEvaluationManager:
    def __init__(self):
        self.eval_dir = _REPO_ROOT / "materials" / "evaluations"
    ...
    def create_batch_template(self, batch_number, evaluator):
        ...
        for thesis_id in batch_info['thesis_ids']:
            template['thesis_evaluations'].append({
                "thesis_id": thesis_id,
                "thesis_title": "TODO",
                "source_text": f"materials/texts/tesis{thesis_id}.txt",
                "wit_lines": "TODO",
                "clauses": []
            })
...
def main():
    ...
    if command == "status":
        ...
        if template:
            output = f"materials/evaluations/{evaluator}/batch_{batch_num}_template.json"
