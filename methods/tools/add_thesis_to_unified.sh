...
THESIS_NUM=$1
THESIS_TITLE=$2
TEXT_FILE="$REPO_ROOT/materials/texts/tesis${THESIS_NUM,,}.txt"
...
# Insert before the comparative tail marker (after all thesis sections)
INSERTION_LINE=$(grep -n "// End of individual thesis sections" "$UNIFIED_FILE" | head -1 | cut -d: -f1)
if [ -z "$INSERTION_LINE" ]; then
    INSERTION_LINE=$(grep -n "// COMPARATIVE ANALYSIS SECTION" "$UNIFIED_FILE" | head -1 | cut -d: -f1)
fi

if [ -z "$INSERTION_LINE" ]; then
    echo "Error: Could not find insertion point (End of individual thesis / COMPARATIVE ANALYSIS)"
    rm "$TEMP_SECTION"
    exit 1
fi
...
echo "4. Test with: $REPO_ROOT/methods/tools/validate_unified.sh"
echo ""
